...
/**
 * Persistent tree map that fully implements Collection interface. Uses wrapper pattern for
...
public class PersistentTreeMap<K extends Comparable<K>, V>
  ...
  public boolean add(Map.Entry<K, V> entry) {
    throw new UnsupportedOperationException(
        "PersistentTreeMap is immutable. Use TransactionalPersistentTreeMap wrapper for mutable"
            + " operations.");
  }
  ...
  private TreeNode<K, V> balance(TreeNode<K, V> node) {
    ...
    if (balanceFactor > 1) {
      if (node.left.balanceFactor() < 0) {
        // Left-Right case
        return rotateRight(new TreeNode<>(node.key, node.value, rotateLeft(node.left), node.right));
      }
      // Left-Left case
    ...
    if (balanceFactor < -1) {
      if (node.right.balanceFactor() > 0) {
        // Right-Left case
        return rotateLeft(new TreeNode<>(node.key, node.value, node.left, rotateRight(node.right)));
      }
      // Right-Right case
  ...
  private TreeNode<K, V> rotateRight(TreeNode<K, V> y) {
    ...
    return new TreeNode<>(x.key, x.value, x.left, new TreeNode<>(y.key, y.value, t2, y.right));
  }
  ...
  private TreeNode<K, V> rotateLeft(TreeNode<K, V> x) {
    ...
    return new TreeNode<>(y.key, y.value, new TreeNode<>(x.key, x.value, x.left, t2), y.right);
  }
  ...
  private class MapIterator implements Iterator<Map.Entry<K, V>> {
    ...
    @Override
    public void remove() {
      throw new UnsupportedOperationException("Persistent structures are immutable");
    }
  }
